		...
		<el-row>
			...
			<el-col :span="15">
				<div class="rating-all">
					<div>
						<span>服务态度</span>
						<el-rate v-model="seller.serviceScore" disabled show-score text-color="#ff9900" score-template="{value}"></el-rate>
					</div>
					<div>
						<span>商品质量</span>
						<el-rate v-model="seller.foodScore" disabled show-score text-color="#ff9900" score-template="{value}"></el-rate>
					</div>
					<div>
						<span>送达时间</span>
...
export default {
	...
	methods:{
		...
		getRatings(rateType){
			this.isActive = rateType;
			this.$http.get("/seller/rating").then(res => {
				//if(res.data.status){
